	...
	var Mapplic = function(element) {
		...
		function Tooltip() {
			...
			this.init = function() {
				...
				$('<p style="display:none;">Close</p>').appendTo($('.mapplic-tooltip-close'));
				this.close.on('click touchend', function(e) {
					e.preventDefault();
					self.hideLocation();
					if (!self.o.zoom || self.o.zoomoutclose) self.moveTo(0.5, 0.5, self.fitscale, 400, 'easeInOutCubic');
				});
				if (this.tooltipthumb) this.thumbnail = $('<img>').addClass('mapplic-tooltip-thumbnail').attr('alt','').hide().appendTo(this.el);
